...
if [[ "$TOOL" == "local" ]]; then
  ...
  echo "1. Read docs/APP_PRD.md, scripts/ralph/prd.json, and scripts/ralph/progress.txt."
  ...
  echo "5. If checks pass, set only $STORY_ID passes=true and add notes in scripts/ralph/prd.json."
  echo "6. Append a progress entry to scripts/ralph/progress.txt."
