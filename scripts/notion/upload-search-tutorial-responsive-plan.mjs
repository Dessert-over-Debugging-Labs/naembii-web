...
const children = [
  paragraph(`작성일: ${today}`),
  heading(2, '목적'),
  paragraph('사용자가 Maangchi 같은 크리에이터를 음식 카드로 오해하지 않도록 검색 결과를 요리와 크리에이터로 분리한다. 랜딩/앱의 행동 요구를 줄이고, 따라해보기는 화면 밖 안내가 아니라 조리 화면 안의 튜토리얼로 경험하게 만든다. iPhone 16 계열과 Galaxy Fold 계열에서 로딩 멈춤, 중복 렌더링, 화면 잘림을 검증한다.'),
  heading(2, '이번 작업 범위'),
  bullet('검색 화면에서 요리 결과와 크리에이터 결과를 시각적으로 분리한다.'),
  bullet('빠른 검색어에서 크리에이터와 요리 카테고리를 섞지 않는다.'),
  ...
  heading(2, '구현 체크리스트'),
  todo('검색 빠른 칩을 요리/상황 중심으로 정리하고 크리에이터 칩은 별도 영역으로 분리'),
  todo('검색 결과 헤더를 요리 결과와 크리에이터 결과로 분리'),
  todo('크리에이터 결과는 작은 행 형태로 보여 요리 카드와 혼동되지 않게 처리'),
  todo('Maangchi 검색 시 레시피 카드와 크리에이터 행이 동시에 보이되, 의미가 분리되도록 처리'),
  ...
  heading(2, '완료 기준'),
  numbered('Maangchi는 음식 카드처럼 보이지 않고 크리에이터 결과로 분리된다.'),
  numbered('요리 결과는 계속 카드로 보이며, 제목/재료/크리에이터 검색이 동작한다.'),
