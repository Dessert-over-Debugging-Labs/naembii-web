...
function reportChildren() {
  ...
  return [
    ...
    todo('짧은 모바일 화면에서 패널 기본 높이를 낮춰 현재 단계 카드와 질문 칩이 보이도록 추가 보정', true),
    heading(3, '검증 결과'),
    todo('npm run check PASS', true),
    todo('npm run verify:mobile-flow PASS', true),
    todo('npm run verify:app-screens PASS · 126 checked / 0 failed', true),
    todo('npm run verify:long-dynamic:smoke PASS · 1 cycle / 6 tasks / 34s', true),
    todo('모바일 스크린샷 직접 확인 완료', true),
    heading(3, '수치 메모'),
    code([
      'assistant direct input exists: false',
      'assistant waiting state: 말하기 준비 또는 준비된 질문 선택',
      'video duck commands: 80 -> 22 -> 80',
      'mobile-short assistant panel: current step visible, quick question chips visible',
      'app-screens output: /tmp/cook-wireframe-v3/final-app-screens-after-compact-panel'
    ].join('\n')),
