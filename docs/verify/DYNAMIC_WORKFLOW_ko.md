...
| `--visual` | 모바일/데스크톱 CDP 캡처, 공개 화면 금지어와 핵심 섹션 확인 |

## 점수표

최종 검증은 게이트가 모두 통과해도 점수 기준을 넘지 못하면 실패로 본다. `npm run verify:visual` 기준 총점은 100점, 통과 기준은 95점 이상이다.
...
| 앱 미리보기 깊이 | 15 | 홈/상세/조리/완료 흐름을 랜딩 안에서 바로 볼 수 있는가 |
| 내부 정보 비노출 | 15 | 공개 화면에 내부 문서명, 버전 태그, 저장소/배포 구현명이 노출되지 않는가 |
| 반응형·시각 증거 | 10 | 모바일/데스크톱 캡처에서 CTA, 앱 미리보기, 카피 신호가 확인되는가 |
| 배포 준비도 | 10 | Vercel 루트, 수집 환경변수, 배포 제외, 검증 스크립트가 준비됐는가 |
| 검증 루프 재현성 | 5 | 다음 세션에서도 같은 점수표와 리포트를 재실행할 수 있는가 |

빠른 검증인 `npm run verify:dynamic`은 시각 항목을 제외하고 90점 이상을 기준으로 본다. 이 모드는 개발 중 빠른 회귀 확인용이고, 사용자 공유 전 최종 판정은 `verify:visual`로 한다.
...
6. 점수표에서 95점 미만이면 게이트가 모두 PASS여도 보정 후 재검증한다.
