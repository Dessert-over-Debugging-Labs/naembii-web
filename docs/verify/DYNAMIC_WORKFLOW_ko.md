...
이 명령은 랜딩 캡처뿐 아니라 `/app` 내부 홈/검색/상세/조리/타이머/재료/음성비서/완료 화면의 잘림 검증도 함께 실행한다. 로컬 서버(`npm run dev`)와 Chrome headless 실행 권한이 필요하다. Chrome/CDP 실행이 막히면 제품 실패가 아니라 `INCONCLUSIVE`로 기록한다. 최종 공유용 검증은 이 명령을 기준으로 한다.

웹앱 내부 화면 잘림만 빠르게 확인할 때:

```bash
npm run verify:app-screens -- http://127.0.0.1:4190 /tmp/cook-wireframe-v3/app-clip
```

이 검증은 390x844, 375x667, 데스크톱 앱 쉘에서 총 30개 상태를 확인한다. 필수 UI가 viewport 밖으로 나가거나, 가로 overflow가 생기거나, 스크롤 불가능한 화면에서 세로 overflow가 생기면 실패한다.
...
| `--visual` | 모바일/태블릿/데스크톱 CDP 캡처, 공개 화면 금지어와 핵심 섹션 확인, `/app` 내부 상태별 clipping/overflow 확인 |
...
| 후킹·상호작용 루프 | 10 | 요리비서 추천, 후기/팁, 완료 공유, 폰 내부 플로팅 피드백, 재료 바텀시트의 기본 목록 보기와 체크리스트 추가 보기가 유지되는가 |
...
| 반응형·시각 증거 | 10 | 모바일/태블릿/데스크톱 캡처에서 CTA, 핵심 GIF, 앱 미리보기, 카피 신호가 확인되고 `/app` 내부 화면이 잘리지 않는가 |
...
| 역할 | 입력 | 판정 기준 |
| --- | --- | --- |
| 구조 검증자 | `app.html`, `package.json`, `vercel.json` | 랜딩 entry, 앱 iframe, 베타/피드백/레시피 요청 DOM 존재, 재료 바텀시트의 기존 목록 보기와 체크리스트 추가 보기 병존 |
| 문구 검증자 | 공개 화면 캡처 또는 `document.body.innerText` | 내부 작업명과 구현 세부 정보 노출 0건 |
| API 검증자 | `api/` 핸들러 | 저장소 환경변수 없이 validation 400 응답 확인 |
| 시각 검증자 | `/tmp/cook-wireframe-v3/dynamic-*.png`, `/tmp/cook-wireframe-v3/app-clip/*.png` | 모바일/데스크톱 첫 화면에서 신청 CTA와 앱 미리보기 확인, `/app` 내부 30개 상태의 필수 UI 잘림/overflow 0건 |
...
- 앱 내부 잘림 검증은 `validate-app-screens.mjs`가 홈/검색/상세/조리/타이머/재료/음성비서/완료 상태를 직접 전환해 필수 UI 위치와 overflow를 확인한다.
