...
| 후킹·상호작용 루프 | 10 | 요리비서 설문, 트렌드/많이 만드는 섹션, 폰 내부 플로팅 피드백, 후기/팁 더미, 완료 공유, 인터랙션 GIF, 재료 바텀시트의 기본 목록 보기+체크리스트 추가 보기 | 검색 아래 피드백 카테고리 제거, 공유/후기/추천 루프 복구, 체크리스트가 기존 재료 보기를 대체하지 않도록 복구 |
...
| 반응형·시각 증거 | 10 | 모바일, 태블릿, 데스크톱 캡처 PASS. 모바일 첫 화면 CTA/GIF 노출, 앱 캡처 가로 스크롤, `/app` 내부 30개 상태 잘림/overflow PASS | 모바일 히어로 높이, proof 카드, 캡처 레이아웃, 앱 내부 화면별 viewport 잘림 수정 |
...
| app screen clipping | `/app` 내부 홈/검색/상세/조리/완료/시트 화면이 모바일에서 잘림 |
