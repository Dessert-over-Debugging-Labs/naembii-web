...
| 앱 내부 화면 잘림 | `/app`의 홈, 검색, 상세, 조리, 타이머, 재료, 음성비서, 완료 화면은 390x844과 375x667 모바일에서 필수 UI가 viewport 안에 있어야 하며 가로 overflow가 생기면 실패다. | `scripts/validate-app-screens.mjs` |
...
| 포인트 | 기준 | 주요 증거 |
| --- | --- | --- |
| 검색 동작 | 요리 제목, 재료, 유튜버/크리에이터 이름, 요약/메타 텍스트로 검색되어야 한다. | `app.html` |
...
| 재료 보기 | 조리 모드의 재료 바텀시트는 기존 카드형 “한눈에 보기”를 기본으로 유지하고, “체크하며 준비”를 추가 보기로 제공해야 한다. 체크리스트가 기존 보기를 대체하면 실패다. | `app.html`, `scripts/validate-mobile-flow.mjs` |
